...
if (!defined('SSWAF_LOG_DIR'))   define('SSWAF_LOG_DIR', SSWAF_DIR . 'logs/');
...
function sswaf_load_rules() {
	...
	if (!file_exists($rules_file) || !is_readable($rules_file)) {
		if (defined('WP_DEBUG') && WP_DEBUG) {
			error_log('SentryOwl: Rules file not found or not readable: ' . $rules_file); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- intentional debug logging
		}
	...
	if (json_last_error() !== JSON_ERROR_NONE) {
		if (defined('WP_DEBUG') && WP_DEBUG) {
			error_log('SentryOwl: JSON parse error: ' . json_last_error_msg()); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- intentional debug logging
		}
		$rules = array('settings' => array(), 'rules' => array());
		return $rules;
	}
...
function sswaf_get_remote_addr() {
	...
	if (!empty($trusted_proxies) && in_array($remote_addr, $trusted_proxies, true)) {
		if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
			...
			$remote_addr = trim($ips[0]);
		} elseif (!empty($_SERVER['HTTP_X_REAL_IP'])) {
			$remote_addr = trim($_SERVER['HTTP_X_REAL_IP']);
		}
	}
	// phpcs:enable WordPress.Security.ValidatedSanitizedInput
...
function sswaf_get_mu_loader_content() {
	...
	$content .= "\tAuto-generated by SentryOwl Firewall. Do not edit.\n";
